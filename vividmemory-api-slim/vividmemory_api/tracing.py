"""
OpenTelemetry distributed tracing instrumentation for VividMemory API.
...
# Provider name mapping (VividMemory internal -> GenAI semantic convention)
...
def create_operation_span(operation: str, bank_id: str | None = None):
    """
    Create a parent span for a VividMemory operation (retain, reflect, consolidation, etc.).

    This creates the span hierarchy:
    - vividmemory.{operation} (parent)
    ...
    span_name = f"vividmemory.{operation}"
    span = _tracer.start_as_current_span(span_name)

    # Add operation-specific attributes
    if span and hasattr(span, "set_attribute"):
        span.set_attribute("vividmemory.operation", operation)
        if bank_id:
            span.set_attribute("vividmemory.bank_id", bank_id)
...
class LLMSpanRecorder:
    ...
    def record_llm_call(
        ...
        **_extra: Any,
        ...
        Args:
            provider: VividMemory provider name
            model: Model name
        ...
        try:
            ...
            # Create span name: "vividmemory.{scope}" for consistency with parent spans
            # Model info is available in span attributes (gen_ai.request.model)
            if scope:
                span_name = f"vividmemory.{scope}"
            else:
            ...
            with self.tracer.start_as_current_span(
                ...
                # Add custom attributes for VividMemory context
                span.set_attribute("vividmemory.scope", scope)
                span.set_attribute("vividmemory.provider.internal", provider)
