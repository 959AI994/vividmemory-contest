...
from vividmemory_api.config import (
...
from vividmemory_api.engine.memory_engine import fq_table
from vividmemory_api.extensions.tenant import TenantExtension
from vividmemory_api.models import RequestContext

if TYPE_CHECKING:
    from vividmemory_api.engine.db.base import DatabaseBackend
...
def _validate_retain_strategy_chunking(base_config: VividMemoryConfig, strategies: Any) -> None:
    """Validate retain strategy chunking with the same semantics as apply_strategy()."""
    if not isinstance(strategies, dict):
        return
    configurable = VividMemoryConfig.get_configurable_fields()
...
class ConfigResolver:
    ...
    def __init__(self, backend: "DatabaseBackend", tenant_extension: TenantExtension | None = None):
        ...
        self._configurable_fields = VividMemoryConfig.get_configurable_fields()
        self._credential_fields = VividMemoryConfig.get_credential_fields()
    ...
    async def resolve_full_config(self, bank_id: str, context: RequestContext | None = None) -> VividMemoryConfig:
        """
        Resolve full VividMemoryConfig for a bank with hierarchical overrides applied.
        ...
        Returns:
            Complete VividMemoryConfig with hierarchical overrides applied
        """
        ...
        resolved_config = VividMemoryConfig(**config_dict)
        ...
        *,
        ...
        Args:
            ...
            updates: Dict of config field names to new values.
                    Keys can be in env var format (VIVIDMEMORY_API_LLM_PROVIDER)
        ...
        if invalid_fields:
            static_fields = VividMemoryConfig.get_static_fields()
        ...
        if chunking_fields_updated:
            ...
            base_config = VividMemoryConfig(**config_dict)
            validate_retain_chunking_config(
...
def apply_strategy(config: VividMemoryConfig, strategy_name: str) -> VividMemoryConfig:
    """
