...
DEFAULT_USERNAME = "vividmemory"
DEFAULT_PASSWORD = "vividmemory"
DEFAULT_DATABASE = "vividmemory"
        ...
        if self._pg0 is None:
            try:
                ...
                raise ImportError(
                    ...
                    "Install it with: pip install 'vividmemory-api-slim[embedded-db]'"
                )
            kwargs = {
                "name": self.name,
...
class Pg0Url:
    ...
    which case the pg0 defaults (``vividmemory``/``vividmemory``) apply.
    """
...
def parse_pg0_url(db_url: str) -> Pg0Url:
    ...
    Supports:
    - "pg0" -> default instance "vividmemory"
    ...
    """
    if db_url == "pg0":
        return Pg0Url(is_pg0=True, instance_name="vividmemory")
    ...
    return Pg0Url(
        is_pg0=True,
        instance_name=instance_name or "vividmemory",
        port=port,
        username=username,
        password=password,
    )
