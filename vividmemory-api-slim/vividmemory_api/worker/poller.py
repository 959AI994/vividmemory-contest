...
if TYPE_CHECKING:
    from vividmemory_api.engine.db.base import DatabaseBackend, DatabaseConnection
...
class WorkerPoller:
    ...
    async def _scan_active_schemas(self, schemas: list[str | None]) -> set[str | None]:
        ...
        ``vividmemory_api.engine.db.optional_routines`` for the canonical
        install SQL.
        """
        async with self._backend.acquire() as conn:
            if await self._optional_routines.is_installed(conn, "schemas_with_pending_work"):
                ...
                # ``vividmemory_api.engine.db.optional_routines``.
    ...
    async def _execute_task_inner(self, task: ClaimedTask, holder: StageHolder | None = None):
        ...
        # failure visibility use the vividmemory_async_operations{status="failed"} gauge,
        ...
        try:
            ...
            message = (
                ...
                f"(stage={stage}) and was cancelled. Raise VIVIDMEMORY_API_RETAIN_WALL_TIMEOUT "
    ...
    async def recover_own_tasks(self) -> int:
        ...
        for schema in schemas:
            try:
                ...
                # toward the retry budget (worker_max_retries / VIVIDMEMORY_API_WORKER_MAX_RETRIES).
    ...
    async def _log_progress_if_due(self):
        ...
        Emits four kinds of lines:
          ...
          * [DB_WAITS]      - any non-idle vividmemory session waiting on a lock
        """
        now = time.time()
    ...
    async def _log_db_waits(self) -> None:
        """Log any non-idle vividmemory session that's waiting on a lock or other resource.
