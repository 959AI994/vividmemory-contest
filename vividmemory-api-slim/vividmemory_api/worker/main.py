"""
Command-line interface for VividMemory Worker.
...
def create_worker_app(poller: WorkerPoller, memory):
    ...
    app = FastAPI(
        title="VividMemory Worker",
        description="Worker process for distributed task execution",
    )

    # Initialize OpenTelemetry metrics
    try:
        prometheus_reader = initialize_metrics(service_name="vividmemory-worker", service_version="1.0.0")
...
def main():
    """Main entry point for the vividmemory-worker CLI."""
    ...
    parser = argparse.ArgumentParser(
        prog="vividmemory-worker",
        description="VividMemory Worker - distributed task processor",
    )
    ...
    parser.add_argument(
        ...
        help="Worker identifier (default: hostname, env: VIVIDMEMORY_API_WORKER_ID)",
    )
    parser.add_argument(
        "--poll-interval",
        type=int,
        default=config.worker_poll_interval_ms,
        help=f"Poll interval in milliseconds (default: {config.worker_poll_interval_ms}, env: VIVIDMEMORY_API_WORKER_POLL_INTERVAL_MS)",
    )
    parser.add_argument(
        "--max-retries",
        type=int,
        default=config.worker_max_retries,
        help=f"Max retries before marking failed (default: {config.worker_max_retries}, env: VIVIDMEMORY_API_WORKER_MAX_RETRIES)",
    )
    ...
    parser.add_argument(
        ...
        help=f"HTTP port for metrics/health endpoints (default: {config.worker_http_port}, env: VIVIDMEMORY_API_WORKER_HTTP_PORT)",
    )
    parser.add_argument(
        "--http-host",
        default="0.0.0.0",
    ...
    parser.add_argument(
        ...
        help=f"Log level (default: {config.log_level}, env: VIVIDMEMORY_API_LOG_LEVEL)",
    )
    ...
    worker_id_source = "VIVIDMEMORY_API_WORKER_ID/--worker-id" if args.worker_id else "hostname (default)"
    logger.info(f"Worker id: {worker_id} (source: {worker_id_source})")

    # Import MemoryEngine here to avoid circular imports
    from .. import MemoryEngine

    print(f"Starting VividMemory Worker: {worker_id}")
    ...
    async def run():
        ...
        from vividmemory_api.config import DEFAULT_DATABASE_SCHEMA
