...
def run_migrations(
    ...
    Args:
        ...
        script_location: Path to alembic migrations directory (e.g., "/path/to/alembic").
                        If None, defaults to vividmemory-api/alembic directory.
    ...
    # VIVIDMEMORY_API_MIGRATION_DATABASE_URL to the direct PostgreSQL endpoint
    # (e.g. vividmemory-pg-rw) to restore correct locking behaviour.
    ...
    try:
        ...
        if script_location is None:
            # Default: use the alembic directory inside the vividmemory_api package
            # This file is in: vividmemory_api/migrations.py
            # Alembic is in: vividmemory_api/alembic/
    ...
    with engine.connect() as conn:
        ...
        if tables_with_data:
            ...
            raise RuntimeError(
                ...
                f"  2. Use the current vector extension (set VIVIDMEMORY_API_VECTOR_EXTENSION='{current_ext_name}')"
            )
    ...
    with engine.connect() as conn:
        ...
        if tables_with_data:
            ...
            raise RuntimeError(
                ...
                f"  2. Use the current text search extension (set VIVIDMEMORY_API_TEXT_SEARCH_EXTENSION='{current_ext}')"
            )
        ...
        for table_name, current_col_type, current_idx_type, _was_pg_search in mismatched_tables:
            ...
            if text_search_extension == "vchord":
                ...
                # VIVIDMEMORY_API_TEXT_SEARCH_EXTENSION_NATIVE_LANGUAGE — see
