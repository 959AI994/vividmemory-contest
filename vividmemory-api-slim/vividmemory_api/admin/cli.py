...
from ..config import DEFAULT_DATABASE_SCHEMA, VividMemoryConfig, load_dotenv_for_entrypoint
...
app = typer.Typer(name="vividmemory-admin", help="VividMemory administrative commands")
...
def backup(
    ...
    """Backup the VividMemory database to a zip file."""
    config = VividMemoryConfig.from_env()

    if not config.database_url:
        typer.echo("Error: Database URL not configured.", err=True)
        typer.echo("Set VIVIDMEMORY_API_DATABASE_URL environment variable.", err=True)
        raise typer.Exit(1)
...
def run_db_migration(
    ...
    skip_extension_reconcile: bool = typer.Option(
        ...
        help=(
            ...
            "work when the configured backend (VIVIDMEMORY_API_VECTOR_EXTENSION / "
            "VIVIDMEMORY_API_TEXT_SEARCH_EXTENSION) differs from a schema's existing indexes — a "
