"""
Daemon mode support for VividMemory API.
...
DEFAULT_IDLE_TIMEOUT = 0  # 0 = no auto-exit (vividmemory-embed passes its own timeout)

# Allow override via environment variable for profile-specific logs
DAEMON_LOG_PATH = Path(os.getenv("VIVIDMEMORY_API_DAEMON_LOG", str(Path.home() / ".vividmemory" / "daemon.log")))

# Internal env var: set by daemonize() in the re-exec'd child so the child
# skips re-exec and just redirects stdio.  Also set by vividmemory-embed's
# DaemonEmbedManager so the daemon launched via Popen skips re-exec entirely
# (vividmemory-embed's Popen already provides a clean, detached process).
ENV_DAEMON_CHILD = "_VIVIDMEMORY_DAEMON_CHILD"
...
def daemonize():
    ...
    The function has two code paths controlled by the ``_VIVIDMEMORY_DAEMON_CHILD``
    ...
    * **Parent** (env var not set): re-exec the same command via Popen with
      ...
      ``_VIVIDMEMORY_DAEMON_CHILD=1``.  Then ``sys.exit(0)``.
    * **Child** (env var set): redirect stdio to the daemon log file and return.
    ...
    redirect stdout/stderr to ``VIVIDMEMORY_API_DAEMON_LOG`` before exec.
    ...
    # call, or launched by vividmemory-embed with the env var set), just redirect
    ...
    cmd = [sys.executable, "-m", "vividmemory_api.main"] + child_args

    env = os.environ.copy()
    env[ENV_DAEMON_CHILD] = "1"
    env["VIVIDMEMORY_API_DAEMON_LOG"] = str(DAEMON_LOG_PATH)
