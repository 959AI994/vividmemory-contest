...
if TYPE_CHECKING:
    ...
    from vividmemory_api.engine.response_models import RecallResult as RecallResultModel
    from vividmemory_api.engine.response_models import ReflectResult
...
class OperationValidatorExtension(Extension, ABC):
    ...
    Configuration is passed from prefixed environment variables:
        VIVIDMEMORY_API_OPERATION_VALIDATOR_MAX_REQUESTS=100
    ...
    Outcomes for `validate_*` hooks:
        ...
        - defer: raise `DeferOperation(exec_date, reason)` from
          `vividmemory_api.worker.exceptions` to requeue the task for a
