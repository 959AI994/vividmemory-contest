"""Base Extension class for all VividMemory extensions."""

from abc import ABC
from typing import TYPE_CHECKING

if TYPE_CHECKING:
    from vividmemory_api.extensions.context import ExtensionContext


class Extension(ABC):
    """
    Base class for all VividMemory extensions.

    Extensions are loaded via environment variables and receive configuration
    from prefixed environment variables.

    Example:
        VIVIDMEMORY_API_MY_EXTENSION=mypackage.ext:MyExtension
        VIVIDMEMORY_API_MY_SOME_CONFIG=value
