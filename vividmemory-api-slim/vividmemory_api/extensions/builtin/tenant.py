...
class DefaultTenantExtension(TenantExtension):
    ...
    VIVIDMEMORY_API_DATABASE_SCHEMA (defaults to 'public').
    ...
    Configuration:
        VIVIDMEMORY_API_DATABASE_SCHEMA=your-schema (optional, defaults to 'public')

    This is automatically enabled by default. To use custom authentication,
    configure a different tenant extension:
        VIVIDMEMORY_API_TENANT_EXTENSION=vividmemory_api.extensions.builtin.tenant:ApiKeyTenantExtension
    """
...
class ApiKeyTenantExtension(TenantExtension):
    ...
    1. Validates the API key matches VIVIDMEMORY_API_TENANT_API_KEY
    2. Returns the configured schema (VIVIDMEMORY_API_DATABASE_SCHEMA, default 'public')
       for all authenticated requests

    Configuration:
        VIVIDMEMORY_API_TENANT_EXTENSION=vividmemory_api.extensions.builtin.tenant:ApiKeyTenantExtension
        VIVIDMEMORY_API_TENANT_API_KEY=your-secret-key
        VIVIDMEMORY_API_DATABASE_SCHEMA=your-schema (optional, defaults to 'public')
        VIVIDMEMORY_API_TENANT_MCP_AUTH_DISABLED=true (optional, disable auth for MCP endpoints)
    ...
    def __init__(self, config: dict[str, str]):
        ...
        if not self.expected_api_key:
            raise ValueError("VIVIDMEMORY_API_TENANT_API_KEY is required when using ApiKeyTenantExtension")
