...
from vividmemory_api.api import page_markdown
from vividmemory_api.api.disconnect import ClientDisconnectCancellationMiddleware, get_scope_cancellation_token
from vividmemory_api.cancellation import OperationCancelledError
from vividmemory_api.engine.audit import (
    AuditEntry,
    AuditLogger,
    AuditLogListResponse,
    AuditLogStatsResponse,
)
from vividmemory_api.engine.llm_trace import LLMRequestListResponse, LLMRequestStatsResponse
from vividmemory_api.extensions import AuthenticationError, BankWriteOperation, PrecheckOperation
...
from vividmemory_api.config import RETAIN_EXTRACTION_MODES
from vividmemory_api.config_resolver import BankConfigPersistenceConflictError
from vividmemory_api.engine.retain.entity_labels import LabelGroup, _migrate_label_group
...
from vividmemory_api.config import get_config
from vividmemory_api.engine.interface import BankTemplateImportWrite
from vividmemory_api.engine.memory_engine import (
    Budget,
    RetainOperationConflictError,
    _current_schema,
    _get_tiktoken_encoding,
)
from vividmemory_api.engine.mental_model_refresh import MentalModelDryRunRefreshResult
from vividmemory_api.engine.providers.none_llm import LLMNotAvailableError
from vividmemory_api.engine.reflect import ReflectToolCallError
from vividmemory_api.engine.response_models import (
...
from vividmemory_api.engine.search.tags import TagGroup, TagsMatch
from vividmemory_api.engine.structured_output import validate_response_schema
from vividmemory_api.extensions import HttpExtension, OperationValidationError, load_extension
from vividmemory_api.metrics import (
...
class BankConfigUpdate(BaseModel):
    ...
    updates: dict[str, Any] = Field(
        ...
        "or environment variable format (VIVIDMEMORY_API_LLM_PROVIDER). "
...
class DocumentResponse(BaseModel):
    ...
    id: str
    bank_id: str
    # None when document text storage is disabled (VIVIDMEMORY_API_STORE_DOCUMENT_TEXT=false).
    ...
    def validate_version(cls, v: str) -> str:
        ...
        if ver > int(BANK_TEMPLATE_CURRENT_VERSION):
            raise ValueError(
                ...
                f"(max supported: {BANK_TEMPLATE_CURRENT_VERSION}). Please upgrade VividMemory."
            )
        return v
...
from vividmemory_api.webhooks.models import WebhookHttpConfig
...
def create_app(
    ...
    Args:
        ...
        http_extension: Optional HTTP extension to mount custom endpoints under /extension/.
                       If None, attempts to load from VIVIDMEMORY_API_HTTP_EXTENSION env var.
    ...
    async def lifespan(app: FastAPI):
        ...
        from vividmemory_api.config import get_config
        from vividmemory_api.worker import WorkerPoller
        ...
        try:
            prometheus_reader = initialize_metrics(service_name="vividmemory-api", service_version="1.0.0")
            create_metrics_collector()
        ...
        if config.otel_traces_enabled:
            if not config.otel_exporter_otlp_endpoint:
                ...
                from vividmemory_api.tracing import create_span_recorder, initialize_tracing
        ...
        if config.worker_enabled and memory._backend.supports_worker_poller:
            ...
            worker_id_source = "VIVIDMEMORY_API_WORKER_ID" if config.worker_id else "hostname (default)"
    ...
    from vividmemory_api import __version__
    from vividmemory_api.config import get_config

    config = get_config()

    app = FastAPI(
        title="VividMemory HTTP API",
        version=__version__,
        description="HTTP API for VividMemory",
        contact={
            "name": "Memory System",
        },
        license_info={
...
def _register_routes(app: FastAPI):
    ...
    def precheck_for(operation: PrecheckOperation):
        ...
        async def _precheck_dep(
            ...
            if validator is None:
                return
            from vividmemory_api.extensions import PrecheckContext
    ...
    async def version_endpoint() -> VersionResponse:
        ...
        """
        from vividmemory_api import __version__
        from vividmemory_api.config import _get_raw_config
    ...
    async def _require_dry_run_enabled() -> None:
        ...
        if not get_config().enable_dry_run_extract:
            raise HTTPException(
                status_code=404,
                detail="Dry-run extraction is disabled. Set VIVIDMEMORY_API_ENABLE_DRY_RUN_EXTRACT=true to re-enable.",
            )
    ...
    @app.post(
        ...
        "VIVIDMEMORY_API_ENABLE_BANK_LLM_HEALTH=false.",
    ...
    async def api_bank_llm_health(bank_id: str, request_context: RequestContext = Depends(get_request_context)):
        ...
        if not get_config().enable_bank_llm_health:
            raise HTTPException(
                status_code=404,
                detail="Bank LLM health check is disabled. Set VIVIDMEMORY_API_ENABLE_BANK_LLM_HEALTH=true to enable.",
            )
        try:
        ...
        try:
            if not get_config().enable_document_export_api:
                raise HTTPException(
                    ...
                    "Set VIVIDMEMORY_API_ENABLE_DOCUMENT_EXPORT_API=true to enable.",
                )
            profile = await app.state.memory.get_bank_profile(
                bank_id, request_context=request_context, create_if_missing=False
            )
        ...
        try:
            if not get_config().enable_document_import_api:
                raise HTTPException(
                    ...
                    "Set VIVIDMEMORY_API_ENABLE_DOCUMENT_IMPORT_API=true to enable.",
                )
    ...
    async def api_get_bank_config(bank_id: str, request_context: RequestContext = Depends(get_request_context)):
        ...
        if not get_config().enable_bank_config_api:
            raise HTTPException(
                status_code=404,
                detail="Bank configuration API is disabled. Set VIVIDMEMORY_API_ENABLE_BANK_CONFIG_API=true to re-enable.",
            )
        try:
    ...
    @app.patch(
        ...
        "Keys can be provided in Python field format (llm_provider) or environment variable format (VIVIDMEMORY_API_LLM_PROVIDER).",
    ...
    async def api_retain(
        ...
        try:
            ...
            if request.async_:
                ...
                if config.retain_batch_enabled:
                    raise HTTPException(
                        status_code=400,
                        detail=(
                            "Batch API is enabled (VIVIDMEMORY_API_RETAIN_BATCH_ENABLED=true) but async=false. "
                            ...
                            "by setting VIVIDMEMORY_API_RETAIN_BATCH_ENABLED=false in your environment."
                        ),
                    )
            ...
            from vividmemory_api.engine.retain.orchestrator import MemoryDefenseAllBlockedError
    ...
    @app.post(
        ...
        "- Falls back to the server default (`VIVIDMEMORY_API_FILE_PARSER`) if not specified.\n"
    ...
    async def api_file_retain(
        ...
        if not config.enable_file_upload_api:
            raise HTTPException(
                status_code=404,
                detail="File upload API is disabled. Set VIVIDMEMORY_API_ENABLE_FILE_UPLOAD_API=true to enable.",
            )
    ...
    @app.get(
        ...
        "Requires LLM request tracing to be enabled (VIVIDMEMORY_API_LLM_TRACE_ENABLED).",
