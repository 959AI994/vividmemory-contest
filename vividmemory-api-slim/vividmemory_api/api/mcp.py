"""VividMemory MCP Server implementation using FastMCP (HTTP transport)."""
...
from vividmemory_api import __version__ as VIVIDMEMORY_VERSION
from vividmemory_api.config import DEFAULT_MCP_RECALL_DESCRIPTION, DEFAULT_MCP_RETAIN_DESCRIPTION, _get_raw_config
from vividmemory_api.engine.memory_engine import _current_schema
from vividmemory_api.extensions import MCPExtension, load_extension
from vividmemory_api.extensions.tenant import AuthenticationError
from vividmemory_api.mcp_tools import _ALL_TOOLS, MCPToolsConfig, register_mcp_tools
from vividmemory_api.models import RequestContext

# Configure logging from VIVIDMEMORY_API_LOG_LEVEL environment variable
_log_level_str = os.environ.get("VIVIDMEMORY_API_LOG_LEVEL", "info").lower()
_log_level_map = {
...
DEFAULT_BANK_ID = os.environ.get("VIVIDMEMORY_MCP_BANK_ID", "default")
...
MCP_AUTH_TOKEN = os.environ.get("VIVIDMEMORY_API_MCP_AUTH_TOKEN")
...
def create_mcp_server(memory: MemoryEngine, multi_bank: bool = True) -> FastMCP:
    """
    Create and configure the VividMemory MCP server.
    ...
    """
    mcp = FastMCP("vividmemory-mcp-server", version=VIVIDMEMORY_VERSION)
...
class MCPMiddleware:
    ...
    Authentication:
        1. If VIVIDMEMORY_API_MCP_AUTH_TOKEN is set (legacy), validates against that token
    ...
    1. Multi-bank mode (for /mcp/ root endpoint):
       ...
       - Bank ID from: X-Bank-Id header or VIVIDMEMORY_MCP_BANK_ID env var
    ...
    Bank ID resolution priority:
        ...
        3. VIVIDMEMORY_MCP_BANK_ID env var → multi-bank mode (default: "default")
