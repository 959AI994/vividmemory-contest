"""
Alembic environment for VividMemory.
...
from vividmemory_api.db_url import is_oracle_url, to_libpq_url
from vividmemory_api.models import Base


def load_env() -> None:
    """Load environment variables from .env (skipped if already configured)."""
    if os.getenv("VIVIDMEMORY_API_DATABASE_URL"):
        return
...
def get_database_url() -> str:
    ...
    if not database_url:
        database_url = os.getenv("VIVIDMEMORY_API_DATABASE_URL")
        if not database_url:
            raise ValueError(
                "Database URL not found. "
                "Set VIVIDMEMORY_API_DATABASE_URL environment variable or pass database_url to run_migrations()."
            )
