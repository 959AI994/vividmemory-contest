"""
Command-line interface for VividMemory API.

Run the server with:
    vividmemory-api

Run as background daemon:
    vividmemory-api --daemon
...
def resolve_daemon_host_port(
    *,
    ...
    via --host flag or VIVIDMEMORY_API_HOST env var. Uses DEFAULT_DAEMON_PORT
...
def _parse_cli_args(argv: list[str], config: VividMemoryConfig) -> ParsedCliArgs:
    parser = argparse.ArgumentParser(
        prog="vividmemory-api",
        description="VividMemory API Server",
    )
    ...
    parser.add_argument(
        ...
        help=f"Host to bind to (default: {config.host}, env: VIVIDMEMORY_API_HOST)",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=argparse.SUPPRESS,
        help=f"Port to bind to (default: {config.port}, env: VIVIDMEMORY_API_PORT)",
    )
    parser.add_argument(
        "--log-level",
...
def main():
    ...
    # daemonize() or by vividmemory-embed's DaemonEmbedManager.  The child
    ...
    uvicorn_config = {
        "app": "vividmemory_api.server:app" if use_import_string else app,
        "host": args.host,
        "port": args.port,
