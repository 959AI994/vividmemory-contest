"""
FastAPI server for VividMemory API.

This module provides the ASGI app for uvicorn import string usage:
    uvicorn vividmemory_api.server:app

For CLI usage, use the vividmemory-api command instead.
"""
...
from vividmemory_api.api import create_app
from vividmemory_api.config import get_config, load_dotenv_for_entrypoint
...
# `uvicorn vividmemory_api.server:app`, and the import string uvicorn re-imports
# in each worker process when `vividmemory-api` runs with --workers/--reload. Load
# .env here (like the CLI entry points) so those worker processes see the same
# configuration. Importing vividmemory_api as a library never reaches this module.
...
if __name__ == "__main__":
    ...
    from vividmemory_api.main import main
