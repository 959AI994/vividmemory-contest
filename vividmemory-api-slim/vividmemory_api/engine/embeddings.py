...
Configuration via environment variables - see vividmemory_api.config for all env var names.
"""
...
class LocalSTEmbeddings(Embeddings):
    ...
    def _encode_local(
        ...
        try:
            ...
            # Qwen3-Embedding get their configured query prompt without VividMemory carrying
    ...
    def __init__(
        ...
        if model_path and tokenizer_name_or_path is None:
            logger.warning(
                ...
                "VIVIDMEMORY_API_EMBEDDINGS_ONNX_TOKENIZER_NAME_OR_PATH when using local ONNX artifacts.",
                model_id,
            )
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install 'vividmemory-api-slim[local-onnx]'"
            ) from exc
        ...
        if not model_path:
            try:
                ...
                raise ImportError(
                    ...
                    "Set VIVIDMEMORY_API_EMBEDDINGS_ONNX_MODEL_PATH or install local-onnx."
                ) from exc
...
class CodexOAuthEmbeddings(OpenAIEmbeddings):
    ...
    Codex OAuth is an LLM-provider auth path in VividMemory, but the same bearer token
...
class ZeroEntropyEmbeddings(Embeddings):
    ...
    input_type. VividMemory stores document-side vectors and uses query-side
...
class GeminiEmbeddings(Embeddings):
    ...
    def _init_vertexai(self, genai) -> None:
        ...
        if not self.vertexai_project_id:
            raise ValueError(
                "VIVIDMEMORY_API_EMBEDDINGS_VERTEXAI_PROJECT_ID (or VIVIDMEMORY_API_LLM_VERTEXAI_PROJECT_ID) "
...
def create_embeddings_from_env() -> Embeddings:
    ...
    if provider == "tei":
        ...
        if not api_key:
            raise ValueError(
                "VIVIDMEMORY_API_EMBEDDINGS_OPENROUTER_API_KEY, VIVIDMEMORY_API_OPENROUTER_API_KEY, "
        ...
        if not api_key:
            raise ValueError(
                "VIVIDMEMORY_API_EMBEDDINGS_REQUESTY_API_KEY, VIVIDMEMORY_API_REQUESTY_API_KEY, "
