...
vividmemory-cloud) when they want the optimisation. When the routine is not
...
Bodies are deliberately not stored here. VividMemory never installs these
...
class OptionalRoutine:
    """One optional server-side routine VividMemory may call when present.

    VividMemory never installs these — operators do, out-of-band. The fields
