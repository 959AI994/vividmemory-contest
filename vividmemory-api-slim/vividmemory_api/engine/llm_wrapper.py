...
# Set VIVIDMEMORY_API_LLM_MAX_CONCURRENT=1 for local LLMs (LM Studio, Ollama).
...
def create_llm_provider(
    ...
    Args:
        ...
        timeout: Per-request LLM timeout in seconds (resolved by the caller from the
            ...
            (``VIVIDMEMORY_API_LLM_TIMEOUT`` / ``DEFAULT_LLM_TIMEOUT`` for those four;
    ...
    if provider_lower == "openai-codex":
        ...
        if not litellmrouter_config:
            raise ValueError(
                ...
                "Set VIVIDMEMORY_API_LLM_LITELLMROUTER_CONFIG (or the per-op variant) "
        ...
        from vividmemory_api.engine.providers.nous_llm import NousLLM
...
class LLMProvider:
    ...
    def __init__(
        ...
        ``VividMemoryConfig``: resolving the server-level default for a ``None`` argument is the
    ...
    async def call(
        ...
        Args:
            ...
            strict_schema: Per-call override requesting grammar-enforced (json_schema strict)
                ...
                inherits the server-level VIVIDMEMORY_API_LLM_STRICT_SCHEMA flag; an explicit
        ...
        # VIVIDMEMORY_API_LLM_STRICT_SCHEMA flag when the caller expressed no
    ...
    def with_config(
        ...
        *,
        ...
        Args:
            config: Resolved ``VividMemoryConfig`` for the current bank/request.
    ...
    def from_env(cls) -> "LLMProvider":
        ...
        # does so without building the full VividMemoryConfig, keeping from_env() a
