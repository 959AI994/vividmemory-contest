...
from vividmemory_api.config import DEFAULT_FILE_PARSER_MARKITDOWN_OCR_PROMPT
        ...
        *,
        ...
        if not model or not model.strip():
            raise ValueError(
                ...
                "Set VIVIDMEMORY_API_FILE_PARSER_MARKITDOWN_OCR_MODEL to an OpenAI-compatible OCR/vision model "
                "with image-input support."
            )
        if not api_key:
            raise ValueError(
                "Markitdown OCR is enabled but no API key is configured. "
                "Set VIVIDMEMORY_API_FILE_PARSER_MARKITDOWN_OCR_API_KEY."
            )
        if not base_url or not base_url.strip():
            raise ValueError(
                "Markitdown OCR is enabled but no base URL is configured. "
                "Set VIVIDMEMORY_API_FILE_PARSER_MARKITDOWN_OCR_BASE_URL to an OpenAI-compatible OCR/vision endpoint."
            )
    ...
    def _convert_sync(self, file_data: bytes, filename: str) -> str:
        ...
        if self._is_image_file(filename) and not self._ocr_enabled:
            raise RuntimeError(
                ...
                "Set VIVIDMEMORY_API_FILE_PARSER_MARKITDOWN_OCR_ENABLED=true and configure an OpenAI-compatible "
