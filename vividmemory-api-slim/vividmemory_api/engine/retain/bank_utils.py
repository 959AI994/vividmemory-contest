...
async def create_bank_vector_indexes(conn, bank_id: str, internal_id: str, ops=None) -> None:
    ...
    Respects the VIVIDMEMORY_API_VECTOR_EXTENSION config to use the appropriate
...
class BankProfileResult:
    ...
    which callers use to drive the one-time VIVIDMEMORY_API_DEFAULT_BANK_TEMPLATE hook.
    """
...
async def get_or_create_bank_profile(pool, bank_id: str) -> BankProfileResult:
    ...
    engine to apply the VIVIDMEMORY_API_DEFAULT_BANK_TEMPLATE hook on first bank
    creation.
