...
def _build_extraction_prompt_and_schema(config) -> tuple[str, type]:
    ...
    from vividmemory_api.engine.prompt_utils import escape_for_prompt
    ...
    # and reflect so VIVIDMEMORY_API_LLM_OUTPUT_LANGUAGE has a uniform effect
    # across the pipeline. This is independent of the BM25 indexing language
    # (VIVIDMEMORY_API_TEXT_SEARCH_EXTENSION_NATIVE_LANGUAGE) by design — search
...
def _build_request_body(llm_config, config, prompt: str, user_message: str, response_schema: type) -> dict:
    ...
    # retain-scoped field, which already folds in the global VIVIDMEMORY_API_LLM_STRICT_SCHEMA
        ...
        try:
            ...
            if any(
                ...
                raise ValueError(
                    ...
                    f"  2. Decrease VIVIDMEMORY_API_RETAIN_MAX_COMPLETION_TOKENS to a value your model supports\n"
...
async def _extract_facts_with_auto_split(
    ...
    Args:
        ...
        config: Resolved VividMemoryConfig for this bank
...
async def extract_facts_from_text(
    ...
    # (see https://github.com/vividmemory/vividmemory/issues/1412).
    tasks = [
        _extract_facts_with_auto_split(
            chunk=chunk,
            chunk_index=i,
...
async def extract_facts_from_contents_batch_api(
    ...
    if not await llm_config._provider_impl.supports_batch_api():
        raise RuntimeError(
            ...
            f"VIVIDMEMORY_API_RETAIN_BATCH_ENABLED and your LLM provider configuration."
        )
