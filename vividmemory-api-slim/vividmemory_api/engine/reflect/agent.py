    ...
    *,
    ...
    for iteration in range(max_iterations):
        ...
        if done_call:
            ...
            from vividmemory_api.tracing import get_tracer

            tracer = get_tracer()
            span_name = "vividmemory.reflect_tool_call"
            with tracer.start_as_current_span(span_name) as span:
                span.set_attribute("vividmemory.scope", "reflect_tool_call")
                span.set_attribute("vividmemory.operation", "reflect_tool_call")
...
async def _execute_tool_with_timing(
    ...
    span_name = f"vividmemory.reflect_tool_exec.{normalized_name}"
    ...
    with tracer.start_as_current_span(
        ...
        # Set attributes
        span.set_attribute("vividmemory.tool.name", normalized_name)
        span.set_attribute("vividmemory.tool.id", tc.id)
        span.set_attribute("vividmemory.tool.arguments", json.dumps(tc.arguments, ensure_ascii=False))
        ...
        try:
            ...
            span.set_attribute("vividmemory.tool.duration_ms", duration_ms)
