...
from vividmemory_api.engine.llm_wrapper import parse_llm_json
from vividmemory_api.engine.providers.llm_debug import dump_request_on_4xx
...
class GeminiLLM(LLMInterface):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # (env: VIVIDMEMORY_API_LLM_EXTRA_BODY).
    ...
    def _init_vertexai(self, **kwargs: Any) -> None:
        ...
        if not project_id:
            raise ValueError(
                "VIVIDMEMORY_API_LLM_VERTEXAI_PROJECT_ID is required for Vertex AI provider. "
    ...
    async def call(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                # Record trace span
                from vividmemory_api.tracing import get_span_recorder
                ...
                from vividmemory_api.tracing import _serialize_for_span
    ...
    async def get_or_create_cached_prefix(
        ...
        *,
        ...
        if self._cache_manager is None:
            ...
            # is actually used.
            from vividmemory_api.engine.providers.gemini_cache import GeminiCacheManager
    ...
    async def submit_batch(
        ...
        uploaded = await self._client.aio.files.upload(
            file=file_obj,
            config=genai_types.UploadFileConfig(mime_type="jsonl", display_name="vividmemory-batch-input"),
        )

        batch = await self._client.aio.batches.create(
            model=self.model,
            src=uploaded.name,
            config=genai_types.CreateBatchJobConfig(display_name="vividmemory-batch"),
        )
    ...
    def _openai_body_to_gemini_request(body: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(response_format, dict) and response_format.get("type") == "json_schema":
            ...
            if schema:
                ...
                # (VIVIDMEMORY_API_LLM_STRICT_SCHEMA=False) get only a textual hint and
