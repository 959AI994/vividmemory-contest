...
Usage:
    VIVIDMEMORY_API_LLM_PROVIDER=llamacpp
    VIVIDMEMORY_API_LLAMACPP_MODEL_PATH=~/.vividmemory/models/gemma-4-E2B-it-Q4_K_M.gguf
    VIVIDMEMORY_API_LLAMACPP_GPU_LAYERS=-1  # -1 = all layers on GPU
    VIVIDMEMORY_API_LLAMACPP_CONTEXT_SIZE=8192
"""
...
from vividmemory_api.engine.response_models import LLMToolCallResult
...
MODELS_DIR = Path.home() / ".vividmemory" / "models"
...
def _download_default_model() -> Path:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install with: pip install 'vividmemory-api-slim[local-llm]'"
        )
    ...
    if model_path:
        ...
        if not p.exists():
            raise FileNotFoundError(
                ...
                f"Set VIVIDMEMORY_API_LLAMACPP_MODEL_PATH to a valid .gguf file, "
...
class LlamaCppLLM(LLMInterface):
    ...
    async def _ensure_initialized(self) -> None:
        ...
        if self._no_grammar:
            logger.info("Grammar enforcement disabled (VIVIDMEMORY_API_LLAMACPP_NO_GRAMMAR=true)")
