...
from vividmemory_api.engine.llm_interface import LLM_TOOL_CHOICE_AUTO, LLMInterface, LLMToolChoice, LLMToolChoiceMode
from vividmemory_api.engine.llm_trace import LLMResponseUsage, stash_response_usage
from vividmemory_api.engine.response_models import LLMToolCall, LLMToolCallResult, TokenUsage
from vividmemory_api.metrics import get_metrics_collector
from vividmemory_api.worker.stage import set_stage
...
# operator-installed plugins (e.g. vividmemory-memory) and their Stop hooks do
...
def _get_isolated_claude_env() -> dict[str, str]:
    ...
    if _isolated_claude_env is None:
        path = tempfile.mkdtemp(prefix="vividmemory-claude-code-")
...
class ClaudeCodeLLM(LLMInterface):
    ...
    async def call(
        ...
        options = ClaudeAgentOptions(
            ...
            # regardless of VIVIDMEMORY_API_*_LLM_MODEL, while metrics/logs still print
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                # Record trace span
                try:
                    from vividmemory_api.tracing import _serialize_for_span, get_span_recorder
    ...
    async def call_with_tools(
        ...
        format to SDK MCP tools, and tool names are formatted as mcp__vividmemory_tools__{name}.
        ...
        allowed_tool_names = [f"mcp__vividmemory_tools__{name}" for name in tool_names]
        mcp_servers_config = {"vividmemory_tools": mcp_server} if sdk_tools else {}
        ...
        if tool_choice.mode is LLMToolChoiceMode.NAMED:
            ...
            forced_tool_mcp_name = f"mcp__vividmemory_tools__{forced_name}"
                            ...
                            if isinstance(message, AssistantMessage):
                                for block in message.content:
                                    if isinstance(block, TextBlock):
                                        ...
                                        # SDK returns tool names with MCP prefix (mcp__vividmemory_tools__{name})
                                        # Strip the prefix to return original tool name expected by caller
                                        tool_name = block.name
                                        if tool_name.startswith("mcp__vividmemory_tools__"):
                                            tool_name = tool_name.replace("mcp__vividmemory_tools__", "", 1)
