...
if TYPE_CHECKING:
    from vividmemory_api.extensions import (
        BankWriteOperation,
...
def _member_to_llm(member: "LLMMemberConfig", config: VividMemoryConfig, defaults: _LLMCallDefaults) -> LLMConfig:
...
def _resolve_reranker_max_candidates(config: VividMemoryConfig, budget: "Budget | None") -> int:
...
def _get_tiktoken_encoding():
    ...
    (see vividmemory_api.engine.token_encoding).
    """
...
def _truncate_query_to_token_limit(query: str, max_query_tokens: int, log_prefix: str = "") -> str:
    ...
    logger.warning(
        ...
        f"raise VIVIDMEMORY_API_RECALL_MAX_QUERY_TOKENS to allow longer queries"
    )
...
class MemoryEngine(MemoryEngineInterface):
    ...
    def __init__(
        ...
        See vividmemory_api.config for environment variable names and defaults.

        Args:
            db_url: PostgreSQL connection URL. Defaults to VIVIDMEMORY_API_DATABASE_URL env var or "pg0".
                    Also supports pg0 URLs: "pg0" or "pg0://instance-name" or "pg0://instance-name:port"
            memory_llm_provider: LLM provider. Defaults to VIVIDMEMORY_API_LLM_PROVIDER env var or "groq".
            memory_llm_api_key: API key for the LLM provider. Defaults to VIVIDMEMORY_API_LLM_API_KEY env var.
            memory_llm_model: Model name. Defaults to VIVIDMEMORY_API_LLM_MODEL env var.
            ...
            pool_min_size: Minimum number of connections in the pool. Defaults to VIVIDMEMORY_API_DB_POOL_MIN_SIZE.
            pool_max_size: Maximum number of connections in the pool. Defaults to VIVIDMEMORY_API_DB_POOL_MAX_SIZE.
            db_command_timeout: PostgreSQL command timeout in seconds. Defaults to VIVIDMEMORY_API_DB_COMMAND_TIMEOUT.
            db_acquire_timeout: Connection acquisition timeout in seconds. Defaults to VIVIDMEMORY_API_DB_ACQUIRE_TIMEOUT.
            ...
            skip_llm_verification: Skip LLM connection verification during initialization.
                                  Defaults to VIVIDMEMORY_API_SKIP_LLM_VERIFICATION env var or False.
        """
        ...
        if not memory_llm_api_key and requires_api_key(memory_llm_provider):
            raise ValueError("LLM API key is required. Set VIVIDMEMORY_API_LLM_API_KEY environment variable.")
        ...
        # (``VIVIDMEMORY_API_RETAIN_LLM_TIMEOUT``, ``..._MAX_RETRIES``, ``..._INITIAL_BACKOFF``,
        ...
        # Configurable via VIVIDMEMORY_API_RECALL_MAX_CONCURRENT (default: 50)
        ...
        # Configurable via VIVIDMEMORY_API_RETAIN_MAX_CONCURRENT (default: 4).
    ...
    async def _validate_operation(self, validation_coro) -> "ValidationResult | None":
        ...
        from vividmemory_api.extensions import OperationValidationError
    ...
    async def _authenticate_tenant(self, request_context: "RequestContext | None") -> str:
        ...
        """
        from vividmemory_api.extensions import AuthenticationError
    ...
    async def _handle_file_convert_retain(self, task_dict: dict[str, Any]):
        ...
        if self._operation_validator:
            try:
                from vividmemory_api.extensions.operation_validator import FileConvertResult
                from vividmemory_api.models import RequestContext
    ...
    async def _handle_refresh_mental_model(self, task_dict: dict[str, Any]):
        ...
        if self._operation_validator:
            from vividmemory_api.extensions.operation_validator import MentalModelRefreshResult
        ...
        async with audit_context(
            ...
            try:
                ...
                if task_type == "file_convert_retain":
                    ...
                    # Retry count and backoff come from config (VIVIDMEMORY_API_WORKER_MAX_RETRIES and
                    # VIVIDMEMORY_API_WORKER_TASK_RETRY_BACKOFF_SECONDS). Defaults of 3 x 60s give a
    ...
    async def _handle_webhook_delivery(self, task_dict: dict[str, Any]) -> None:
        ...
        headers: dict[str, str] = {
            ...
            "X-VividMemory-Event": event_type,
            **http_config.headers,
        }
        if secret and self._webhook_manager:
            headers["X-VividMemory-Signature"] = self._webhook_manager._sign_payload(secret, payload_bytes)
    ...
    async def _mark_operation_completed(self, operation_id: str):
        ...
        Opt-in escape hatch: when ``VIVIDMEMORY_API_FAIL_ON_EXTRACTION_ERRORS`` is set and the
                ...
                async with conn.transaction():
                    ...
                    if fail_on_errors and extraction_errors_count > 0:
                        error_message = (
                            ...
                            "marked failed because VIVIDMEMORY_API_FAIL_ON_EXTRACTION_ERRORS is enabled. "
    ...
    async def _maybe_update_parent_operation(self, child_operation_id: str, conn):
        ...
        try:
            ...
            if any_failed:
                ...
                # cause correctly.
                from vividmemory_api.worker.poller import _summarise_child_error_messages
    ...
    async def initialize(self):
            ...
            if not self._skip_llm_verification:
                ...
                if config.retain_batch_enabled:
                    ...
                    if not supports_batch:
                        raise RuntimeError(
                            f"Configuration error: VIVIDMEMORY_API_RETAIN_BATCH_ENABLED=true "
                            ...
                            f"set VIVIDMEMORY_API_RETAIN_BATCH_ENABLED=false."
                        )
        ...
        # See https://github.com/vividmemory/vividmemory/issues/2162.
        ...
        if llama_parse_key:
            ...
            logger.debug("LlamaParse parser not registered (VIVIDMEMORY_API_FILE_PARSER_LLAMA_PARSE_API_KEY not set)")
    ...
    def retain(
        ...
        from vividmemory_api.models import RequestContext as RC
    ...
    async def retain_batch_async(
        ...
        *,
        ...
        if self._operation_validator:
            from vividmemory_api.extensions import RetainContext
        ...
        if any(item.get("update_mode") == "append" for item in contents):
            ...
            if not bank_cfg.get("store_document_text", DEFAULT_STORE_DOCUMENT_TEXT):
                raise ValueError(
                    ...
                    "(store_document_text / VIVIDMEMORY_API_STORE_DOCUMENT_TEXT) is disabled: the prior "
        ...
        # Call post-operation hook if validator is configured
        if self._operation_validator:
            from vividmemory_api.extensions import RetainResult
    ...
    async def _submit_post_insert_maintenance(
        ...
        bank_id: str,
        request_context: "RequestContext",
        config: VividMemoryConfig | None = None,
    ) -> None:
    ...
    async def _resolve_retain_chunking_config(
        ...
        actually assigns.
        """
        from vividmemory_api.config_resolver import apply_strategy
    ...
    async def export_documents_async(
        ...
        See :mod:`vividmemory_api.engine.transfer`. Embeddings and database ids are
    ...
    async def import_bank_async(
        ...
        *,
        ...
        if self._operation_validator and await bank_utils.get_bank_profile_if_exists(backend, bank_id) is None:
            from vividmemory_api.extensions import CreateBankContext
    ...
    async def recall_async(
        ...
        *,
        ...
        if invalid_types:
            from vividmemory_api.extensions.operation_validator import OperationValidationError
        ...
        # Validate operation if validator is configured
        if self._operation_validator:
            from vividmemory_api.extensions import RecallContext
        ...
        recall_span_context = tracer.start_as_current_span("vividmemory.recall")
        recall_span = recall_span_context.__enter__()
        recall_span.set_attribute("vividmemory.bank_id", bank_id)
        recall_span.set_attribute("vividmemory.query", query[:100])
        recall_span.set_attribute("vividmemory.fact_types", ",".join(fact_type))
        recall_span.set_attribute("vividmemory.thinking_budget", thinking_budget)
        recall_span.set_attribute("vividmemory.max_tokens", max_tokens)
                ...
                for attempt in range(max_retries + 1):
                    try:
                        ...
                        if is_connection_error and attempt < max_retries:
                            ...
                            error_msg = str(e)
                            if self._operation_validator:
                                from vividmemory_api.extensions.operation_validator import RecallResult
    ...
    async def _search_with_retries(
        ...
        try:
            ...
            embedding_span = tracer_otel.start_span("vividmemory.recall_embedding")
            embedding_span.set_attribute("vividmemory.bank_id", bank_id)
            embedding_span.set_attribute("vividmemory.query", query[:100])
            ...
            retrieval_span = tracer_otel.start_span("vividmemory.recall_retrieval")
            retrieval_span.set_attribute("vividmemory.bank_id", bank_id)
            retrieval_span.set_attribute("vividmemory.fact_types", ",".join(fact_type))
            retrieval_span.set_attribute("vividmemory.thinking_budget", thinking_budget)
            ...
            fusion_span = tracer_otel.start_span("vividmemory.recall_fusion")
            fusion_span.set_attribute("vividmemory.bank_id", bank_id)
            fusion_span.set_attribute("vividmemory.semantic_count", len(semantic_results))
            fusion_span.set_attribute("vividmemory.bm25_count", len(bm25_results))
            fusion_span.set_attribute("vividmemory.graph_count", len(graph_results))
            fusion_span.set_attribute("vividmemory.temporal_count", len(temporal_results) if temporal_results else 0)
            ...
            try:
                ...
                fusion_span.set_attribute("vividmemory.merged_count", len(merged_candidates))
                fusion_span.end()
            ...
            rerank_span = tracer_otel.start_span("vividmemory.recall_rerank")
            rerank_span.set_attribute("vividmemory.bank_id", bank_id)
            rerank_span.set_attribute("vividmemory.candidates_count", len(merged_candidates))
            ...
            try:
                ...
                rerank_span.set_attribute("vividmemory.scored_count", len(scored_results))
                if pre_filtered_count > 0:
                    rerank_span.set_attribute("vividmemory.pre_filtered_count", pre_filtered_count)
                rerank_span.end()
    ...
    async def get_document(
        ...
        *,
        ...
        await self._authenticate_tenant(request_context)
        if self._operation_validator:
            from vividmemory_api.extensions import BankReadContext, BankReadOperation
    ...
    async def delete_document(
        ...
        *,
        ...
        await self._authenticate_tenant(request_context)
        if self._operation_validator:
            from vividmemory_api.extensions import BankWriteContext, BankWriteOperation
    ...
    async def _ensure_bank_exists(
        ...
        *,
        ...
        The ``VIVIDMEMORY_API_DEFAULT_BANK_TEMPLATE`` hook is best-effort, opens
    ...
    async def bank_template_import_authorization(
        ...
        *,
        ...
        if self._operation_validator:
            from vividmemory_api.extensions import BankWriteContext
            from vividmemory_api.extensions.operation_validator import MentalModelGetContext, MentalModelRefreshContext
    ...
    def _consume_preauthorized_config_update(
        ...
        from vividmemory_api.extensions import BankWriteOperation
    ...
    async def _validate_bank_config_updates(
        ...
        *,
        ...
        if "memory_defense" in updates and updates["memory_defense"] is not None:
            from vividmemory_api.extensions import OperationValidationError
            from vividmemory_api.extensions.memory_defense import parse_policy
        ...
        if not bank_exists:
            from vividmemory_api.api.http import load_default_bank_template_manifest
    ...
    async def _apply_default_bank_template(
        ...
        """Apply VIVIDMEMORY_API_DEFAULT_BANK_TEMPLATE to a freshly-created bank.
        ...
        from vividmemory_api.api.http import (
        ...
        try:
            ...
            logger.error(
                "VIVIDMEMORY_API_DEFAULT_BANK_TEMPLATE failed schema validation "
                f"and will be ignored for bank '{bank_id}': {'; '.join(errors)}"
            )
            return
        except ValueError as e:
            logger.error(
                "VIVIDMEMORY_API_DEFAULT_BANK_TEMPLATE failed semantic validation "
        ...
        try:
            ...
            logger.info(f"Applied VIVIDMEMORY_API_DEFAULT_BANK_TEMPLATE to newly-created bank '{bank_id}'")
        except Exception as e:
            logger.error(f"Failed to apply VIVIDMEMORY_API_DEFAULT_BANK_TEMPLATE to bank '{bank_id}': {e}")
    ...
    async def list_banks(
        ...
        *,
        ...
        if self._operation_validator:
            from vividmemory_api.extensions import BankListContext
    ...
    async def reflect_async(
        ...
        *,
        ...
        if self._reflect_llm_config is None:
            raise ValueError("Memory LLM API key not set. Set VIVIDMEMORY_API_LLM_API_KEY environment variable.")
        ...
        if self._reflect_llm_config.provider == "none":
            ...
            raise LLMNotAvailableError(
                ...
                "Set VIVIDMEMORY_API_LLM_PROVIDER to a real provider (e.g., openai, anthropic, gemini)."
            )
        ...
        # Validate operation if validator is configured
        if self._operation_validator:
            from vividmemory_api.extensions import ReflectContext
        ...
        try:
            ...
            from vividmemory_api.engine.response_models import DirectiveRef
            ...
            from vividmemory_api.engine.response_models import TokenUsage
            ...
            # Call post-operation hook if validator is configured
            if self._operation_validator:
                from vividmemory_api.extensions.operation_validator import ReflectResultContext
    ...
    async def _compute_bank_stats(self, bank_id: str) -> dict[str, Any]:
        ...
        async with acquire_with_retry(backend) as conn:
            ...
            # UI does not read them; vividmemory-cli `bank stats` does, and after
    ...
    async def get_mental_model(
        ...
        *,
        ...
        if self._operation_validator:
            from vividmemory_api.extensions.operation_validator import MentalModelGetContext
        ...
        if result and self._operation_validator:
            from vividmemory_api.extensions.operation_validator import MentalModelGetResult
    ...
    async def _execute_mental_model_refresh(
        ...
        *,
        ...
        # Skip creating a nested "vividmemory.reflect" span since we already have "vividmemory.mental_model_refresh"
    ...
    async def dry_run_refresh_mental_model(
        ...
        *,
        ...
        if self._operation_validator:
            from vividmemory_api.extensions.operation_validator import MentalModelRefreshContext
