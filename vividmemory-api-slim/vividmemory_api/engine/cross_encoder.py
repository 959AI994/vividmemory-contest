...
class JinaMLXCrossEncoder(CrossEncoderModel):
    ...
    async def initialize(self) -> None:
        ...
        # See: https://github.com/vividmemory/vividmemory/issues/994
        import transformers
...
class AlibabaCloudCrossEncoder(CrossEncoderModel):
    ...
    for qwen3-rerank. Authentication via VIVIDMEMORY_API_RERANKER_ALIBABA_API_KEY
...
class MultiCrossEncoder(CrossEncoderModel):
    ...
    Member 0 is the primary (the unindexed ``VIVIDMEMORY_API_RERANKER_*`` config);
...
def create_cross_encoder(member: RerankerMemberConfig) -> CrossEncoderModel:
    ...
    ``member`` is the primary (index 0, the unindexed ``VIVIDMEMORY_API_RERANKER_*``
    ...
    if provider == "tei":
        ...
        if not api_key:
            shared = ", VIVIDMEMORY_API_OPENROUTER_API_KEY, or VIVIDMEMORY_API_LLM_API_KEY" if member.index == 0 else ""
            raise ValueError(
        ...
        if not project_id:
            shared = " (or VIVIDMEMORY_API_LLM_VERTEXAI_PROJECT_ID)" if member.index == 0 else ""
            raise ValueError(
...
def create_cross_encoder_from_env() -> CrossEncoderModel:
    ...
    With no ``VIVIDMEMORY_API_RERANKER_<n>_*`` members configured (the default) this
