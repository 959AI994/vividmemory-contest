...
def create_file_storage(
    ...
    **kwargs,
    ...
    if storage_type == "native":
        ...
        if not bucket:
            raise ValueError("VIVIDMEMORY_API_FILE_STORAGE_S3_BUCKET is required for S3 storage")
        ...
        if not bucket:
            raise ValueError("VIVIDMEMORY_API_FILE_STORAGE_GCS_BUCKET is required for GCS storage")
        ...
        if not container:
            raise ValueError("VIVIDMEMORY_API_FILE_STORAGE_AZURE_CONTAINER is required for Azure storage")
        ...
        raise ValueError(
            ...
            "External backends load through VIVIDMEMORY_API_FILE_STORAGE_EXTENSION."
        )
...
def _load_file_storage_extension() -> FileStorage | None:
    ...
    ``VIVIDMEMORY_API_FILE_STORAGE_EXTENSION=module.path:ClassName`` names a
    :class:`FileStorage` implementation; every other ``VIVIDMEMORY_API_FILE_STORAGE_*``
    ...
    store uses via ``VIVIDMEMORY_API_MEMORIES_EXTENSION``. Returns ``None`` when no
    extension is configured, so the built-in backends below stay the default.
    """
    import importlib
    import os

    ext_path = os.getenv("VIVIDMEMORY_API_FILE_STORAGE_EXTENSION")
    if not ext_path:
        return None
    if ":" not in ext_path:
        raise ValueError(
            f"Invalid VIVIDMEMORY_API_FILE_STORAGE_EXTENSION '{ext_path}'. Expected format 'module.path:ClassName'."
        )
    ...
    prefix = "VIVIDMEMORY_API_FILE_STORAGE_"
    config = {
        key[len(prefix) :].lower(): value
        for key, value in os.environ.items()
        if key.startswith(prefix) and key != "VIVIDMEMORY_API_FILE_STORAGE_EXTENSION"
    }
    return cls(config)
