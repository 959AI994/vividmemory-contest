...
:class:`~vividmemory_api.engine.memories.postgres.PostgresMemories` can delegate
rather than embed SQL.

Every function takes the live connection and VividMemory's ``fq_table`` resolver, so
...
the guarantee :class:`~vividmemory_api.engine.memories.base.ScanPage` documents:
