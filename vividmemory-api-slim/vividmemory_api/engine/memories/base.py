...
The default :class:`~vividmemory_api.engine.memories.postgres.PostgresMemories`
...
An alternative implementation is loaded like any other VividMemory extension::

    VIVIDMEMORY_API_MEMORIES_EXTENSION=mypackage.memories:MyMemories
    VIVIDMEMORY_API_MEMORIES_SOME_SETTING=value
...
class MemoryPatch:
    ...
    unit_id: str
    text: str | None = None
    # Either a float list or the pgvector literal '[0.1,0.2,...]' — VividMemory
...
class FactRecord:
    ...
    text: str
    # A float list, or the pgvector literal '[0.1,...]' — VividMemory produces both.
    ...
    # Entity names + spelled-out date tokens VividMemory folds into its BM25 document.
    ...
    def metadata_bag(self) -> dict[str, str]:
        ...
        # VividMemory filters recall's created_after/created_before window on
...
def build_text_signals(fact) -> str | None:
    """Entity names + spelled-out dates — the enrichment VividMemory folds into BM25.
...
class MemoriesExtension(Extension, ABC):
    ...
    ``self.config`` (the ``VIVIDMEMORY_API_MEMORIES_*`` environment) and
    ``self.context`` from :class:`~vividmemory_api.extensions.base.Extension`.
