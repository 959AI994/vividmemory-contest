...
Resolved through the ordinary extension loader — ``VIVIDMEMORY_API_MEMORIES_EXTENSION``
names a ``module:Class``, and ``VIVIDMEMORY_API_MEMORIES_*`` becomes its config — so
this behaves like every other extension point. Unset (the normal case) means
:class:`~vividmemory_api.engine.memories.postgres.PostgresMemories`: rows in
