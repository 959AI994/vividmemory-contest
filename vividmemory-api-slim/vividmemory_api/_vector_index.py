...
# Extensions a user can set via VIVIDMEMORY_API_VECTOR_EXTENSION.
...
#   VividMemory's built-in vchord clause does not set lists, so the safe default
...
def configured_vector_extension() -> str:
    ...
    Reads ``VIVIDMEMORY_API_VECTOR_EXTENSION`` (default ``"pgvector"``) and
    ...
    """
    return validate_extension(os.getenv("VIVIDMEMORY_API_VECTOR_EXTENSION", "pgvector"))
